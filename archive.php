...
<?php $this->need('header.php'); ?>
	...
	<?php
	if($this->is('category')){$this->archiveTitle('','分类<span>','</span>的文章');}
	elseif($this->is('tag')){$this->archiveTitle('','标签<span>','</span>的文章');} 
	elseif($this->is('search')){$this->archiveTitle('','搜索<span>','</span>的文章');}
	else{$this->archiveTitle('','<span>','</span>的文章');}
	?>
	</div>
    <?php if ($this->have()): ?>
	<?php while($this->next()): ?>
